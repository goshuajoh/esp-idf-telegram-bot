...
static int consecutive_null_responses = 0;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            printf("Consecutive ++");
                ...
                {
                    ESP_LOGI(TAG, "Entering light sleep after %d consecutive null responses", consecutive_null_responses);
                    esp_sleep_enable_timer_wakeup(5 * 1000 * 1000); // 5 seconds sleep
                    esp_light_sleep_start();

                    esp_wifi_connect(); // Reconnect Wi-Fi after waking up
                    consecutive_null_responses = 0;
                }
                else
                {
                    longPoll = (longPoll < 120) ? longPoll + 5 : 120; // Increase polling interval up to a max of 60 seconds
                }
        }
